...
namespace WildBlueIndustries
{
    ...
    public class WBIPackingBox : WBIMultiConverter, IModuleInfo
    {
        ...
        {
            if (string.IsNullOrEmpty(partToolTip))
                return;

            WBIPathfinderScenario scenario = WBIPathfinderScenario.Instance;
            if (scenario == null || scenario.HasShownToolTip(getMyPartName()))
                return;

            WBIToolTipWindow toolTipWindow = new WBIToolTipWindow(partToolTipTitle, partToolTip);
        ...
        {
            return isDeployed ? "Assembled" : "Packed";
        }
